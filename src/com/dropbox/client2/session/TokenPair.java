    ...
    implements Serializable
{
    ...
    public TokenPair(String s, String s1)
    {
        ...
        if (s.contains("|"))
        {
            throw new IllegalArgumentException((new StringBuilder()).append("'key' must not contain a \"|\" character: \"").append(s).append("\"").toString());
        }
        if (s1 == null)
        {
    ...
    {
        return (new StringBuilder()).append("{key=\"").append(key).append("\", secret=\"").append(secret.charAt(0)).append("...\"}").toString();
    }
}
